...
#         target_datetime = dt_datetime.strptime(target_datetime_str, '%d-%m-%Y %H:%M:%S')
...
#     # Get the current time and round it down to the nearest 15 minutes
#     now = dt_datetime.now()
#     rounded_time = round_down_to_nearest_15_minutes(now)

#     # Create the target datetime string using the rounded time
#     target_datetime_str = rounded_time.strftime('%d-%m-%Y %I:%M %p')  # Use 12-hour format with AM/PM
